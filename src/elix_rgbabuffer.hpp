#ifndef ELIX_RGBAIMAGE_CANVAS_HPP
#define ELIX_RGBAIMAGE_CANVAS_HPP
...
struct rbgabuffer_command_array {
	float * array = nullptr;
	uint32_t index = 0;
	uint32_t max = 0;
};
...
struct rbgabuffer_colour_stop {
	float pos;
	uint32_t col;
};

struct rbgabuffer_linear_gradient {
	uint32_t count = 0;
	rbgabuffer_colour_stop stops[16]; // TODO
};

struct rbgabuffer_radial_gradient {
	uint32_t count = 0;
	rbgabuffer_colour_stop stops[16]; // TODO
};
struct rbgabuffer_pattern {
...
struct rbgabuffer_context {
	...
	char font_family[16] = "Sans-Serif";
	float font_size_px = 20.0;
};

struct elix_text_metrics {
	double width;
};
...
#endif // ELIX_RGBAIMAGE_CANVAS_HPP
