...
uint16_t elix_package__scan( elix_package * pkg ){
	...
	do {
		...
		elix_package_stored_file * info = ALLOCATE(elix_package_stored_file, 1);
...
bool elix_package__read_header(elix_package * pkg, elix_package_type type ) {
	...
	if ( type == EP_AUTO ) {
		for (uint8_t q = EP_RESOURCES;q < EP_AUTO; q++) {
			if ( memcmp(pkg->magic, elix_package_magics[q], 6) ==0 ) {
...
elix_package * elix_package_create( const char * filename, elix_package_type type ) {
	...
	elix_package * pkg = ALLOCATE(elix_package, 1);

	if ( elix_file_open(&pkg->file, filename, EFF_FILE_READ, nullptr) ) {
...
void elix_package_stored_file__destroy(data_pointer * data) {
	...
	if ( info ) {
		if ( info->compressed.data )
			NULLIFY(info->compressed.data);
		if ( info->raw.data )
			NULLIFY(info->raw.data);
	}
	NULLIFY(info);
...
elix_package_data elix_package_get_file( elix_package * pkg, const char * file ){
	...
	if ( info ) {
		if ( info->raw.size ) {
			...
			buffer.data = ALLOCATE(uint8_t, buffer.size);
			...
			if ( info->raw.data ) {
				...
				if ( !info->compressed.data ) {
					info->compressed.data = ALLOCATE(uint8_t, info->compressed.size);
