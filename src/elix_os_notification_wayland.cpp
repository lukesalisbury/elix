...
elix_window_notification_settings elix_window_notification_settings_create( char * subject,  char * action_name, char* action_text, char* body, ... ) {
	...
	settings.icon = elix_cstring_copy("dialog-information");
	if ( action_text && action_name ) {
		settings.action_text = elix_cstring_copy(action_text);
		settings.default_action = elix_cstring_copy(action_name);
	}

	settings.body = elix_cstring_copy(body);
	settings.summary = elix_cstring_copy(subject);
