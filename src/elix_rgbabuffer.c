...
typedef struct rgbabuffer_edge_type {
	float ymin, ymax, xval, slope;
} rgbabuffer_edge_type;

typedef struct rgbabuffer_active_edge_index {
	uint32_t index;
	float xval;
} rgbabuffer_active_edge_index;
...
void rbgabuffer__pixel(rbgabuffer_context * ctx, uint32_t colour, int32_t x , int32_t y, UNUSEDARG float alpha) {
	...
	if ( alpha < 1.0 ) {
		...
		final_colour.a = 255;
		final_colour.r = (uint8_t)((final_colour.r * alpha) + (source_colour.r * (1.0 - alpha)));
		final_colour.g = (uint8_t)((final_colour.g * alpha) + (source_colour.g * (1.0 - alpha)));
		final_colour.b = (uint8_t)((final_colour.b * alpha) + (source_colour.b * (1.0 - alpha)));
...
void rbgabuffer__line(elix_v32_2 p0, elix_v32_2 p1, rbgabuffer_context * ctx, uint32_t colour) {
	...
	rbgabuffer__pixel( ctx, colour, x0, y0, 1.0 );
	#define myPixel( d, a, b, c) rbgabuffer__pixel(a, d, b, c, 1.0);
	if (dx > dy) {
		int length = (dx - 1) >> 2;
		int extras = (dx - 1) & 3;
		int incr2 = (dy << 2) - (dx << 1);
		if (incr2 < 0) {
			...
			for (int i = 0; i < length; i++) {
				...
				if (d < 0) {
					rbgabuffer__pixel(ctx, colour, x0, y0, 1.0 );
					rbgabuffer__pixel(ctx, colour, x0 += stepx, y0, 1.0 );
					rbgabuffer__pixel(ctx, colour, x1, y1, 1.0 );
					rbgabuffer__pixel(ctx, colour, x1 -= stepx, y1, 1.0 );

					d += incr1;
				} else {
					if (d < c) {
						rbgabuffer__pixel(ctx, colour, x0, y0, 1.0 );
						rbgabuffer__pixel(ctx, colour, x0 += stepx, y0 += stepy, 1.0 );
						rbgabuffer__pixel(ctx, colour, x1, y1, 1.0 );
						rbgabuffer__pixel(ctx, colour, x1 -= stepx, y1 -= stepy, 1.0 );
					} else {
						rbgabuffer__pixel(ctx, colour, x0, y0 += stepy, 1.0 );
						rbgabuffer__pixel(ctx, colour, x0 += stepx, y0, 1.0 );
						rbgabuffer__pixel(ctx, colour, x1, y1 -= stepy, 1.0 );
						rbgabuffer__pixel(ctx, colour, x1 -= stepx, y1, 1.0 );
					}
					d += incr2;
				}
...
uint32_t rgbabuffer_active_edge_insert_sorted(rgbabuffer_active_edge_index * arr, rgbabuffer_active_edge_index key, size_t count, size_t capacity)
{
	if ( count >= capacity )
...
uint32_t rgbabuffer_edge_type_insert(rgbabuffer_edge_type * arr, rgbabuffer_edge_type key, size_t count, size_t capacity)
{
	if ( count >= capacity )
		return UINT32_MAX; //ERROR
...
void rbgabuffer_Fill(rbgabuffer_context * ctx) {
	...
	elix_v32_2 y_limits, x_limits; // TODO
...
//	printf("     %.0f%*c%.0f\n", x_limits.x, (int)(x_limits.y-x_limits.x)-1, ' ', x_limits.y  );
	for (float line = y_limits.x; line < y_limits.y; line += 1.0f) {
		...
		rgbabuffer_active_edge_index edge_buffer;
		for (uint32_t c = 0; c < point_count; c++) {
			if ( (uint32_t)globaledges[c].ymin == line32 && globaledges[c].ymin < globaledges[c].ymax) {
				edge_buffer = (rgbabuffer_active_edge_index){ c, globaledges[c].xval };
				rgbabuffer_active_edge_insert_sorted(activeedges, edge_buffer, q++, 64);
			}
		}
		activeedges[q].index = UINT32_MAX;
		activeedges[q].xval = 0.0;
		max_active = q;
		...
		for (float col = x_limits.x; col <= x_limits.y; col += 1.0f) {
			...
			if (!parity)
				rbgabuffer__pixel(ctx, ctx->fill_colour, col32, line32, 1.0 );
...
rgbabuffer_font * rbgabuffer__unloadFont(rbgabuffer_context * ctx, rgbabuffer_font * font) {
	//elix_databuffer_free(&font->data.data);
...
rgbabuffer_font * rbgabuffer__loadFont(rbgabuffer_context* ctx, const char * font_name) {
	...
	if ( raw_file.size ) {
		font = ALLOCATE(rgbabuffer_font, 1);
		font->data = raw_file;
...
rbgabuffer_context * rbgabuffer_create_context(elix_graphic_data * external_buffer, const elix_uv32_2 requested_dimensions ) {
	rbgabuffer_context * context = ALLOCATE(rbgabuffer_context, 1);
